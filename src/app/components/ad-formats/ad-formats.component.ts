    ...
    this.webService.getFormatByUrl(url.format).then(format => {
      ...
      if (url.tag) {
        this.webService.getTagByUrl(this.format[0].id, url.tag).then(tag => {
          ...
          this.webService.getTemplates(this.format[0].id, this.tag[0].id).then(templates => this.templates = templates);
        });
      } else {
        this.webService.getTemplates(this.format[0].id, 0).then(templates => this.templates = templates);
      }
